...
namespace Manga.Core.Infrastructure.DependencyManagement
{
    public class ContainerManager
    {
        ...
        public T Resolve<T>(string key = "", ILifetimeScope scope = null, params Autofac.Core.Parameter[] parameters) where T : class
        {
            if (scope == null)
            {
                //no scope specified
        ...
        public object Resolve(Type type, ILifetimeScope scope = null, params Autofac.Core.Parameter[] parameters)
        {
            if (scope == null)
            {
                //no scope specified
            ...
            return scope.Resolve(type, parameters);
        }

        public T[] ResolveAll<T>(string key = "", ILifetimeScope scope = null, params Autofac.Core.Parameter[] parameters)
        {
            if (scope == null)
            {
                //no scope specified
            ...
            if (string.IsNullOrEmpty(key))
            {
                return scope.Resolve<IEnumerable<T>>(parameters).ToArray();
            }
            return scope.ResolveKeyed<IEnumerable<T>>(key, parameters).ToArray();
        }
